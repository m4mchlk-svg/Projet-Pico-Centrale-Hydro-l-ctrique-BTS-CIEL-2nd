...
lora_pupitre.config['ADDR_H']  = addr_high_pupitre  # adresse barrage MSB
lora_pupitre.config['ADDR_L']  = addr_low_pupitre   # adresse barrage LSB
...
def timer_handler(timer_send_data):
...
timer_send_data.init(mode=Timer.PERIODIC, period=5000, callback=timer_handler)
...
while True :
    if emission_flag :
        print(Color.G)
        lora_pupitre.envoyer_trame(addr_high_barrage, addr_low_barrage, CANAL)
        emission_flag = False
