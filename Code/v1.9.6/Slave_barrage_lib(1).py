...
class LoRaE32:
    ...
    def send_data(self, dest_addr_high, dest_addr_low, dest_channel, data: bytes):
        self._set_mode(0)
        # Attendre AUX HIGH avant d'émettre (module vraiment prêt)
        if self.aux:
            timeout = time.ticks_ms()
            while not self.aux.value():
                if time.ticks_diff(time.ticks_ms(), timeout) > 2000:
                    print("Timeout AUX pré-émission !")
                    return False
                time.sleep_ms(10)
        packet = bytes([dest_addr_high, dest_addr_low, dest_channel]) + data
        self.uart.write(packet)
        # AUX passe à 0 pendant l'émission RF, attendre qu'il repasse à 1
        time.sleep_ms(10)   # laisser le temps à AUX de tomber à 0
        if self.aux:
            timeout = time.ticks_ms()
            while not self.aux.value():
                if time.ticks_diff(time.ticks_ms(), timeout) > 3000:
                    print("Timeout AUX post-émission !")
                    return False
                time.sleep_ms(10)
        # Purger les octets parasites (écho header ou octets d'état module)
        time.sleep_ms(20)
        while self.uart.any():
            self.uart.read()
        return True
    ...
    def crc16(self, texte):  # Contrôle de redondance cyclique : détecter erreurs de transmission
        crc = 0xFFFF
        for char in texte:
            crc ^= ord(char)
            for _ in range(8):
    ...
    def crc_valide(self, texte): # calcul le crc16 de la trame reçue et le compare à celui transmis
        crc_recu = texte[-4:]
    ...
    def parse_message(self, texte): # Analyse de la trame reçue et validée
    ...
    def get_trame(self):   # Création d'une trame sous la forme: $ val3 # val4 # texte2 #  
    ...
    def envoyer_trame(self, addr_high, addr_low, CANAL):
        print("\n*** Trame à envoyer ***")
        trame_emision = self.get_trame()        # Sous la forme: "  $ val3 # val4 # texte2 #  "
        crc = self.crc16(trame_emision)         # calcul du crc16 de la trame à émettre
        message = trame_emision + crc      # Sous la forme: "  $ val1 # val2 # texte # crc16  "
        self.send_data(addr_high, addr_low, CANAL, message.encode('utf-8'))  
        print("Message brut envoyé :", message.encode('utf-8'))

    def lecture_trame(self):
        ...
        if msg:
            try :
                ...
                if trame_reception.startswith('$') and self.crc_valide(trame_reception):
                    resultat = self.parse_message(trame_reception)     # trame valide à analyser
                    return resultat
                return "erreur"
            except:
                return None 
