...
timer_config = Timer(2)

# Configuration du module Lora
lora_barrage.config['CHANNEL'] = CANAL              # fréquence 868 MHz (canal 6)
lora_barrage.config['ADDR_H']  = addr_high_barrage  # adresse pupitre MSB
lora_barrage.config['ADDR_L']  = addr_low_barrage   # adresse pupitre LSB
...
# interruption logicielle du timer 2 :
def lecture_config(timer_config):
    global config_flag   # Pas de traitement long ici !
    config_flag = True  # données à envoter
...
def lecture_trame():
    msg = lora_barrage.get_data() # Lecture du recepteur LoRa 
    if msg:
        try :
...
timer_config.init(mode=Timer.PERIODIC, period=20000, callback=lecture_config)
...
while True:
    ...
    # *** vérification périodique de la config (toutes les 20s) ***
    if config_flag :
        # On utilise l'objet lora_barrage instancié plus haut
        if not lora_barrage.check_config() :
            print(f"{Color.R}Tentative de reconfiguration forcée...{Color.END}")
            lora_barrage.set_config(save=True) # Correction du nom : lora_barrage
        else:
            print(f"{Color.G}Vérification terminée : Tout est OK.{Color.END}")
        
        # IMPORTANT : On reset le flag pour attendre les prochaines 20 secondes
        config_flag = False
