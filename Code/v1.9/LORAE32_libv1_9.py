...
class LORAE32:
    def __init__(self, uart, m0_pin, m1_pin, aux_pin):
    ...
    def setup_config(self,head, ad_h, ad_l, sped, ad_chanel, option):
        self.set_mode(3)
        config_cmd = bytes([head, ad_h, ad_l, sped, ad_chanel, option])
        ...
        if self.uart.any():
            ...
            print(data)
        self.set_mode(0)
        print(f"Configuration fixe appliquée sur canal {ad_chanel}")
    ...
    def send_trame(self, ad_mo_h, ad_mo_l, ad_mo_chanel, flag):
        data_send  = self.get_trame()
        crc = self.crc16(data_send)
        trame_send = data_send + crc
        self.send_point_to_point_txt(ad_mo_h, ad_mo_l, ad_mo_chanel, trame_send)
        print(trame_send)
        print("\n")
        flag = False
        
    def receive_trame(self, compteur_trame_valide, flag):
        data = self.receive()
        if data :
    ...
    def affichage_dict(self, resultat):
